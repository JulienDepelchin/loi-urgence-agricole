...
- Utilise des **onglets** pour basculer entre la vue "Recherche" (liste) et la vue "Carte", plutôt que d'empiler les deux verticalement — ça double la hauteur sinon.
...
- Contours des 2 départements (`departements_59_62.geojson`) affichés en superposition (trait fin, pas de remplissage) pour le repère géographique. Au clic sur un département : popup avec les agrégats sénatoriaux (`nb_senateurs`, `votes_pour`, `votes_contre`, `votes_abstention`) et la liste des sénateurs de ce département (obtenue en filtrant `elus_votes.json` sur `qualite === "Sénateur"` et `code_departement`), chacun avec son vote.
...
- Typographie : titres en serif (Georgia ou équivalent système), corps en sans-serif système, chiffres des compteurs en police tabulaire (`font-variant-numeric: tabular-nums`).
